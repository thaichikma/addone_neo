...
class AbstractInventoryReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, partner):
        ...
        sheet.merge_range("Y8:AG8", delivery.scheduled_date, header_right)
        ...
        for report in result:
            ...
            sheet.merge_range(row, 23, row + 2, 32, report[5], quantity)
            row += 3
            stt += 1
        ...
        sheet.write(row + 5, 2, "2.1", bottun_center)
        sheet.write(row + 6, 2, "2.2", bottun_center)
        sheet.write(row + 7, 2, "2.3", bottun_center)
        ...
        sheet.merge_range(row + 8, 2, row + 9, 13, "NIKKISO VIETNAM, INC.", bottun_center)
        sheet.merge_range(row + 8, 19, row + 9, 30, "NEOTECH SOLUTION JSC", bottun_center)
        sheet.merge_range(row + 10, 2, row + 11, 13, "We hereby agree and confirm our acceptance of this order", bottun_center)
