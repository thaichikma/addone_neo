...
See bellow Legends for supported Date and Time Formats.
    ...
    description = fields.Text('Displayed Description',
    ...
    report_excel_param_ids = fields.One2many('report.excel.param', 'report_excel_id', string='Report Parameters',
                                             copy=False)
    report_id = fields.Integer(compute='_compute_id', string="Report id", store=False)
    report_excel_param_content = fields.Char(compute='_compute_report_excel_param_content', string="Report Parameters ",
                                             store=False)
    report_excel_section_ids = fields.One2many('report.excel.section', 'report_excel_id', string='Report Sections',
                                               copy=True)
    ...
    def create_action(self):
        for report in self:
            ...
            action_id = self.env['ir.actions.act_window'].search(
                ["&", ('res_model', '=', 'report_excel_wizard'), "|", ('context', '=', context_report1),
        ...
        if 'report_excel_param_ids' in vals:
            ...
            for param_id in res.report_excel_param_ids:
                if param_id.wizard_param_ir_model_field_id.id == False:
                    ...
                    vals_update['report_excel_param_ids'].append(
        ...
        if 'report_excel_param_ids' in vals:
            ...
            for param in vals['report_excel_param_ids']:
                if param[0] in [1, 2, 3, 5]:
                    vals_update_before['report_excel_param_ids'].append(
    ...
    def unlink(self):
        ...
        if not len(wizard_param_ir_model_field_ids):
            records = self.env['ir.model.fields'].search(
                [('name', 'like', 'x_param_'), ('model', '=', 'report_excel_wizard')], [])
            for r in records:
...
class ReportExcelSection(models.Model):
    ...
    report_id = fields.Integer(compute='_compute_report_id', string="Report id", default=_default_compute_report_id,
                               store=False)
    report_excel_param_content = fields.Char(compute='_compute_report_excel_param_content', string="Report Parameters",
                                             default=_default_report_excel_param_content, store=False)
    ...
    report_excel_fields_ids = fields.One2many('report.excel.fields', 'report_excel_section_id',
    ...
    report_excel_fields_sql_ids = fields.One2many('report.excel.fields.sql', 'report_excel_section_id',
...
class ReportExcelFields(models.Model):
    ...
    report_excel_section_id = fields.Many2one('report.excel.section', string='Section', ondelete='cascade',
...
\u0023   result = cell(H7) ''',
    ...
    def _onchange_having_param_domain(self):
        ...
        if self.field_type in ('date',) and self.aggregate_id.code != 'count':
            return {
                'domain': {'having_param_id': [('report_excel_id', '=', report_id), ('type_param', 'in', ('date',))]}}
        if self.field_type in ('date',) and self.aggregate_id.code == 'count':
            return {'domain': {
                'having_param_id': [('report_excel_id', '=', report_id), ('type_param', 'in', ('integer', 'float',))]}}
        elif self.field_type in ('datetime',) and self.aggregate_id.code != 'count':
            return {'domain': {
                'having_param_id': [('report_excel_id', '=', report_id), ('type_param', 'in', ('datetime',))]}}
        elif self.field_type in ('datetime',) and self.aggregate_id.code == 'count':
            return {'domain': {
                'having_param_id': [('report_excel_id', '=', report_id), ('type_param', 'in', ('integer', 'float',))]}}
        else:
    ...
    def get_all_field_of_m(self, azz):
        a = azz
        print(a)
        ...
        for menu in self:
            ...
            action_id = self.env['ir.actions.act_window'].search(
                ["&", ('res_model', '=', 'report_excel_wizard'), "|", ('context', '=', context_report1),
                 ('context', '=', context_report2)]).id
            if not action_id:
                report_excel_name = self.env['report.excel'].search(
